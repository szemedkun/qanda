...
class Fit2(object):
	def __init__(self, model = recurrent.LSTM, w2v_dim = 50, sent_hidden_size = 50, 
				query_hidden_size = 10, batch_size = 16, epochs = 20, vocab_size = None, rs = False
				, sent_hidden_size2 = 20, query_hidden_size2 = 5, two_hidden_layers = False):
		'''
	...
	def compile_layers(self):
		...
		if self.two_hidden_layers:
			...
			# merging
			model = Sequential()
			model.add(Merge([sentrnn1, sentrnn2, qrnn], mode='concat'))

			# output layer
			model.add(Dense(2*self.SENT_HIDDEN_SIZE + self.QUERY_HIDDEN_SIZE, self.vocab_size, activation='softmax'))

			model.compile(optimizer='adam', loss='categorical_crossentropy', class_mode='categorical')
			self.model = model


	def run(self, X1, X2, Xq, Y):
		'''
		Ones I am done with exploration, I will make this more flexible

		'''
		print('Training')
		self.model.fit([X1, X2, Xq], Y, batch_size=self.BATCH_SIZE, nb_epoch=self.EPOCHS, validation_split=0.05, show_accuracy=True)

	def score(self,tX1, tX2, tXq, tY):
		'''
		Come back to this and make it flexible

		Right now it returns accuracy
		'''
		loss, acc = self.model.evaluate([tX1, tX2, tXq], tY, batch_size=self.BATCH_SIZE, show_accuracy=True)
