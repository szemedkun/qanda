...
model = Fit(vocab_size = ds.answers_size
	, sent_hidden_size = max( [ max([2 * X.shape[1] // 3, 50]), 2*X.shape[1] // 3 ] )
	, epochs = 20
	, batch_size = 32 )
...
task19_pred = compare_predictions(ds, model, tX, tXq, tY)

with open('../../capstone_web/static/pickles/task19_pred.pkl', 'wb') as f:
	pkl.dump(task19_pred, f)
